...
namespace Tangent.Parsing.UnitTests
{
    ...
    public class PartialStatementParseTests
    {
        ...
        public void LambdasPullLeadingIdentifier()
        {
            ...
            Assert.AreEqual("x", l.Takes.First().ParameterDeclaration.Takes.First().Identifier.Identifier.Value);
        ...
        public void LambdasPullManyVardecls()
        {
            ...
            Assert.AreEqual(null, l.Takes.First().ParameterDeclaration.Returns);
            Assert.AreEqual("x", l.Takes.First().ParameterDeclaration.Takes.First().Identifier.Identifier.Value);
            Assert.AreEqual("y", l.Takes.First().ParameterDeclaration.Takes.Skip(1).First().Identifier.Identifier.Value);
            Assert.AreEqual(null, l.Takes.First().ParameterDeclaration.Returns);
            Assert.AreEqual("x", l.Takes.First().ParameterDeclaration.Takes.First().Identifier.Identifier.Value);
            Assert.AreEqual("y", l.Takes.First().ParameterDeclaration.Takes.Skip(1).First().Identifier.Identifier.Value);
            Assert.AreEqual("z", l.Takes.First().ParameterDeclaration.Takes.Skip(2).First().Identifier.Identifier.Value);
            Assert.AreEqual(null, l.Takes.First().ParameterDeclaration.Returns);
        }
    }
}
